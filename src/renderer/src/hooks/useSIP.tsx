    ...
    (destination: string) => {
      ...
      if (!remoteAudio) {
        ...
        if (newAudio) {
          ...
          console.warn(
            'Could not create audio element, proceeding with call but audio may not work'
          )
        }
      }
      ...
      try {
        ...
        // Không dùng extra headers - có thể gây lỗi với một số server SIP
        const callOptions = {
          ...DEFAULT_CALL_OPTIONS,
        ...
        console.log('Call initiated with early media support')
      } catch (error) {
