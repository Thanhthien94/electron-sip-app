...
export const CDRList = () => {
  ...
  const {
    ...
    isLoading
  } = useCDRApi(searchParams, user?.token ?? null, [user?.token ?? null, callState])
  ...
  const handleNextPage = () => {
    const nextPage = () => ({ ...searchParams, page: searchParams.page + 1 })
    setSearchParams(nextPage())
  }

  const handlePrevPage = () => {
    const prevPage = () => ({ ...searchParams, page: searchParams.page - 1 })
    ...
    <div className="w-full h-full flex flex-col">
      ...
      <div className="flex flex-col gap-3 items-center w-full overflow-y-auto h-[calc(100vh-220px)] px-2">
        ...
        {!isLoading && listCDR.length === 0 && (
          <div className="text-center text-gray-400 py-8">Không có dữ liệu cuộc gọi</div>
        )}
      </div>
      ...
      {count > 0 && (
        <div className="flex flex-row gap-2 items-center text-xs mx-auto mt-2 w-fit">
          ...
          <button
            ...
            className="hover:text-sky-600 text-orange-400"
          ...
          <button
            ...
            className="hover:text-sky-600 text-orange-400"
            disabled={count < searchParams.limit}
          >
